...
### v2.5.44 (2026-06-09)
- 修复：恢复「全部」页按中文平台名查询，避免只显示 GitHub、36Kr 等少数渠道
- 测试：新增全部页平台分组回归测试，防止平台 ID/显示名映射再次漂移
- 验证：确认 compose 稳定 MySQL 数据目录与运行容器挂载一致

### v2.5.43 (2026-06-09)
- 部署：固定 compose 使用稳定 MySQL 数据目录，避免本地路径变化导致数据库重新初始化
- 优化：提升底部版本号对比度，确保页脚版本信息更易读
- 验证：同步本地端口、镜像 revision 与 GitHub main 状态，并确认 Web/API/MCP 服务可用

### v2.5.42 (2026-06-04)
- 维护：补齐源级缓存记录状态字段，记录每个来源的成功、失败时间与状态
- 验证：保持新闻抓取、前端构建和 diff 检查通过

### v2.5.41 (2026-06-04)
- 维护：为源级缓存记录平台字段补充索引，提升按来源查询一致性
- 验证：保持构建、测试和 diff 检查通过

### v2.5.40 (2026-06-04)
- 优化：对齐 source-first 刷新路径，恢复参考来源抓取链路
- 验证：保持构建、测试和 diff 检查通过

### v2.5.39 (2026-06-04)
- 修复：恢复参考消息抓取逻辑，保持已验证新闻获取链路可用
- 验证：保持前端构建和 diff 检查通过

### v2.5.38 (2026-06-03)
- 修复：新闻接口自动刷新改为等待刷新后返回，减少页面首次请求返回旧数据

### v2.5.37 (2026-06-02)
- 维护：同步已验证的系统审计优先级修复，补充自动刷新回归测试和 Agent 项目指令
- 运维：忽略 SQLite journal 临时文件，并在 Docker 发布工作流中跳过 PR 场景 registry 登录
- 验证：完成移动端/桌面端首页、全部页、登录注册和设置弹窗人工检查，未发现阻断问题

### v2.5.36 (2026-06-02)
- 修复：飞书推送改用 `post` 富文本消息体，保留分组、编号与可点击链接格式
- 修复：恢复“全部”标签页渠道新闻序号索引，避免序号显示为 `NaN`

### v2.5.35 (2026-06-02)
- 修复：补齐 CI 中 `pytest` 安装，避免 GitHub Actions 在 `Run pytest` 步骤因缺少依赖失败
- 验证：确认修复后远端 `CI` 与 `Docker Image` 工作流均已触发并通过

### v2.5.34 (2026-06-02)
- 维护：提交并发布已验证的维护修复，归档 legacy 旧实现并移除已跟踪运行时文件
- 部署：Dockerfile 显式声明 `16888` 和 `8000` 端口，补充独立 CI 工作流校验构建、测试、安全扫描与平台一致性
- 优化：清理新闻列表未使用的 `index`，保持移动端界面与前端构建产物一致
