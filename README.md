...
- UI：重构为浅色优先、跟随系统深色模式的 Liquid Glass 液态玻璃设计系统。
- UI：统一页面背景光斑、玻璃表面、渐变高光线、卡片、按钮、表单和弹窗视觉语言。
- UI：补齐深色模式下的文本、边框、分隔线和交互状态，避免硬编码浅色导致可读性下降。
- 体验：增加 `prefers-reduced-motion` 与表单焦点态适配，并保留原有业务逻辑与数据流程不变。
### v2.5.66 (2026-09-06)
...
- 新增：调度状态 API（`/api/scheduler`），支持查询运行状态、暂停/恢复、调整刷新间隔
- 新增：抓取层进程级互斥锁，避免手动/定时/自动刷新并发重复抓取
- 新增：RSS 源支持 ETag/Last-Modified 条件请求，304 保留旧数据并记录未变更状态
- 新增：前端统一 axios 封装（自动注入 token、401 清理登录态、403/404/500 全局提示）与全局 Toast 组件
- 新增：GitHub Actions 每日热点归档（`archive/YYYY-MM-DD.json`），零服务器留档
- 重构：后端环境变量统一使用 pydantic-settings 管理，配置非法启动即报错
- 修复：定时推送“无新内容”不再累计失败退避；前端 403/404/500 错误避免重复弹窗

### v2.5.64 (2026-06-15)
- 修复：关键词标签页因模板调用缺失函数 `formatDisplayTime` 导致渲染异常而显示空白

### v2.5.63 (2026-06-14)
- 修复：关键词标签页正确渲染关键词分组列表，避免点击标签后因平台分组分支抢先命中而出现空白
- 调整：刷新状态增加按来源返回的缓存状态，便于前端识别最新、缓存和失败状态
- 对齐：百度热搜链接改用 NewsNow 同源 rawUrl，FastAPI 启停逻辑迁移到 lifespan

### v2.5.62 (2026-06-12)
- 修复：实时页按 NewsNow 口径优先使用条目发布时间展示与排序，兼容“刚刚 / N分钟前 / HH:mm / 完整时间”等来源格式

### v2.5.61 (2026-06-12)
- 调整：API 返回 `updated_at` 字段，为前端时间兜底提供统一字段

### v2.5.60 (2026-06-12)
- 调整：热榜页按渠道统一时间展示，有发布时间的渠道统一显示发布时间，避免同渠道混用抓取时间

### v2.5.59 (2026-06-12)
- 调整：关键词标签页默认仅展示当天入库的匹配新闻，避免旧新闻继续出现在标签页
- 测试：新增标签页当天过滤回归测试

### v2.5.58 (2026-06-12)
- 调整：36Kr 来源区分为 `36Kr快讯` 与 `36Kr热榜`，避免快讯和热榜混在同一渠道展示
- 对齐：实时页将 `36Kr快讯` 归入实时渠道，`36Kr热榜` 保持热榜渠道展示

### v2.5.57 (2026-06-11)
- 修复：成功抓取到当前热榜时替换该平台旧列表，避免近 7 天历史新闻继续混入首页展示
- 对齐：参考消息按 NewsNow 的 `publishTime` 合并排序，三个频道整体倒序后取前 20 条
- 保留：抓取失败或返回空时不清空旧缓存，继续作为兜底展示
- 验证：68 项测试、安全扫描、平台一致性、Docker Compose 配置和前端构建均通过

### v2.5.56 (2026-06-11)
- 修复：新增 pytest 配置，将测试收集范围固定为 `tests/`，避免 CI 或本地运行 `python -m pytest -q` 时误扫描 MySQL 数据目录
- 验证：`python -m pytest -q`、安全扫描、平台一致性和前端构建均通过
...
- 增强：AIHOT 与 Product Hunt 支持主 API 失败或为空时降级 RSS，避免直接静默空结果
- 验证：66 项测试、安全扫描、平台一致性、现场蜘蛛探测和前端构建均通过

### v2.5.52 (2026-06-10)
- 修复：实时时间线对 `pub_time` 为非标准字符串（`"60分钟前"`、`"11:05"`等）的渠道回退到 `created_at`，确保所有渠道均有时间戳

### v2.5.51 (2026-06-10)
- 优化：实时卡片内部改为时间线风格（相对时间 + 左侧竖线标记），热榜保持序号排名不变
- 验证：前端构建、平台一致性（29 个来源）、57 项测试全部通过

### v2.5.50 (2026-06-10)
- 优化：将微博热榜平台名从"微博"改为"微博热搜"，为后续区分子榜单（娱乐、社会等）铺路
- 修复：关键词筛选（标签页）按标题去重，避免同一标题从多个平台命中关键词时重复展示
- 修复：关键词分组（keyword_groups）内同标题只出现一次
- 新增：36氪人气榜、雪球热门股票、快手 3 个热榜来源（向 NewsNow 对齐）
- 验证：平台一致性 29 个来源、57 项测试、前端构建全部通过

### v2.5.49 (2026-06-10)
- 优化：实时标签页改为与热榜页一致的渠道卡片分组展示，不再把所有实时新闻混合成单列表
- 优化：实时页数量统计按渠道分组汇总，渠道卡片显示“实时源 · N条”
- 验证：前端构建、平台一致性和差异检查通过

### v2.5.48 (2026-06-10)
- 修复：凤凰网 `hotNews1` 在部分部署环境为空或页面结构异常时，增加链接兜底解析，避免只保留旧的单条专题数据
- 优化：凤凰网抓取增加关键诊断日志，记录响应状态、HTML 长度、`allData` 命中情况，便于定位不同设备/CDN 返回差异
- 测试：补充凤凰网 `hotNews1` 为空和缺失 `allData` 的兜底回归测试

### v2.5.47 (2026-06-10)
- 新增：全部页拆分为热榜与实时两个标签页，实时页按时间线倒序展示最新信息
- 新增：按 NewsNow 实时源补充金十数据、联合早报、格隆汇、法布财经和远景论坛
- 修复：凤凰网抓取改为解析 NewsNow 同源的 `allData.hotNews1`，避免仅显示少量专题内容
- 优化：实时页排除热榜源，热榜页保留原平台卡片分组展示
- 验证：后端编译、安全扫描、平台一致性、完整测试、Docker Compose 配置、前端构建和差异检查均通过

### v2.5.46 (2026-06-10)
- 新增：按 NewsNow 同源补充财联社电报、B站热门视频和 B站排行榜来源
- 优化：移除微博硬编码备用 Cookie，仅通过 `WEIBO_COOKIE` 环境变量注入，避免敏感凭据进入仓库
- 优化：数据库保留近 7 天新闻历史，同平台同标题同 URL 只更新不重复插入
- 修复：推送内容按标题去重，避免相同标题在一次推送中重复出现
- 验证：保持后端编译、平台一致性、数据库/推送测试和前端构建通过

### v2.5.45 (2026-06-09)
- 修复：对齐微博热榜抓取入口与榜单顺序，减少与 NewsNow 的同源差异
- 优化：刷新模型改为按来源缓存与单源刷新，并在平台卡片新增刷新按钮
- 验证：保持后端测试、前端构建和 diff 检查通过
